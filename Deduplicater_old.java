...
public class Deduplicater {
    ...
    public void removeNonAlphanumericEntries()
    {
        //  False is the argument here because we only want to match 
        remove("[^[0-9A-Z]]*", false);
        remove("[^[0-9a-z]]*", false);
    }
    ...
    //  Insensitive = case insensitive and insensitive to start site
    ...
    {       
        ...
        for (int id = 0; id <= maxId; id++)
        {
            ...
            if (idea != null)
            {
                ...
                if (myMatcher.find())
                {
                    if (myMatcher.start() == 0 || insensitive)
